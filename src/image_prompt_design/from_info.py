...
class ImageDescriptionFromInfo(BaseImagePromptDesigner):
    def __init__(self, disease_name, results_path=None):
        BaseImagePromptDesigner.__init__(self, disease_name=disease_name, results_path=results_path)
        self.info = self.read_info_json()

    def __get_landscape_description(self, i, agent_name, request_name):
        name = "{0}".format(str(i).zfill(3))
        request = self.get_request_if_done(name)
        if request is not None:
            return {"name": name, "content": request}
        system_prompt = """
        I want you to describe a landscape based on text provided by the user.
        You need to can use the style of Ryszard Kapuściński, a Polish journalist and writer known for his literary reportages, to describe the landscape.
        ...
        """
        user_prompt = "Describe a landscape that captures the essence of the text provided:\n"
